...
epoch = 3000  # The number of games we want the system to run for.
...
class CatchEnvironment():
    def __init__(self, gridSize):
        ...
        self.state = np.empty(4, dtype=np.uint8)
    ...
    def drawState(self):
        ...
        if (self.state[2]>5):
            canvas[self.state[0] - 1, self.state[1] - 1] = 1  # Draw the fresh fruit.
        else:
            canvas[self.state[0] - 1, self.state[1] - 1] = 10  # Draw the old fruit.
        
        # Draw the basket. The basket takes the adjacent two places to the position of basket.
        canvas[self.gridSize - 1, self.state[3] - 1 - 1] = 1
        canvas[self.gridSize - 1, self.state[3] - 1] = 1
        canvas[self.gridSize - 1, self.state[3] - 1 + 1] = 1
        return canvas
    ...
    def reset(self):
        ...
        randvalue=random.randrange(1, 11)# random fruit, old fruit
        self.state = np.array([1, initialFruitColumn,randvalue,initialBucketPosition])
    ...
    def getReward(self):
        ...
        if (kind > 5):
            ...
            if (fruitRow == self.gridSize - 1):  # If the old fruit has reached the bottom.
                if (abs(fruitColumn - basket) <= 1):  # Check if the basket caught the fruit.
                    return -1  # 썩은 과일 피하기
                else:
                    return 1 
    ...
    def updateState(self, action):
        ...
        self.state = np.array([fruitRow, fruitColumn, kind,newBasket])
